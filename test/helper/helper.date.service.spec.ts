...
describe('HelperDateService', () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [HelperDateService, ConfigService],
    }).compile();
  ...
  describe('check', () => {
    ...
    it('should be success', () => {
      const isCheck = helperDateService.check(date1.toISOString());
      jest.spyOn(helperDateService, 'check').mockImplementation(() => isCheck);

      expect(helperDateService.check(date1.toISOString())).toBe(isCheck);
    });
  });
