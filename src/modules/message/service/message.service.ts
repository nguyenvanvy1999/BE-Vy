...
import type { IMessage, IMessageOptions, IMessageSetOptions } from '../message.interface';
...
export class MessageService {
  ...
  getRequestErrorsMessage(requestErrors: ValidationError[], appLanguages?: string[]): IErrors[] {
    const messages: IErrors[][] = [];

    for (const transformer of requestErrors) {
      let children: TypeOfObj[] = transformer.children || [];
      let constraints: string[] = transformer.constraints ? Object.keys(transformer.constraints) : [];
  ...
  get(key: string, options?: IMessageOptions): string | IMessage {
    const { properties, appLanguages } = options ? options : { properties: undefined, appLanguages: undefined };
  ...
  getLanguages(): string[] {
    return Object.values(EMessageLanguage);
  }
}
