...
export function ResponsePagingInterceptor(messagePath: string, customStatusCode: number): Type<NestInterceptor> {
  @Injectable()
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<Promise<any> | string> {
      ...
      return next.handle().pipe(
        map(async (response: Promise<TypeOfObj>) => {
          ...
          const { totalData, totalPage, currentPage, perPage, data, availableSort, availableSearch } = responseData;
