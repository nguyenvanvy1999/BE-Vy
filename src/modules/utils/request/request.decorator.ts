...
export const UserAgent = createParamDecorator((data: string, ctx: ExecutionContext): IResult => {
  const { userAgent } = ctx.switchToHttp().getRequest();

  return userAgent;
});

export function RequestParamGuard(...classValidation: Array<ClassConstructor<any>>): ApplyDecorator {
  return applyDecorators(UseGuards(ParamGuard(classValidation)));
}

export const HttpApiRequest = (summary: string, description?: string): ApplyDecorator =>
  applyDecorators(
